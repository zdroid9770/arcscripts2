			...
			if(GetPhase() == 5)
			{
				...
				else if(IsTimerFinished(mEventTimer))
				{
					for(int i = 0; i < 4; ++i)
					{
						...
						CreatureAI* pAI = SpawnCreature(mAdvCoords[i].addition, mAdvCoords[i].x, mAdvCoords[i].y, mAdvCoords[i].z, false);
						if(pAI != NULL)
						{
						...
						if(pCreature != NULL)
						{
							...
							{
								TO< CreatureAI* >(pCreature->GetScript())->AggroNearestUnit(200);
							}
						}
					}
